 ...
 return (
   <section>
    <div className='mx-auto max-w-7xl p-6 lg:px-8'>
     <h1 className='text-4xl font-bold pb-3'>Your Results</h1>
     <h2 className='pb-3'>
      Trip dates:
      <span className='ml-2'>
     ...
     <h3 className='font-semibold text-xl'>
      Results for {results.content.total_listings.match(/^[^:]+/)[0]}
     </h3>
     <div className='space-y-2 mt-5'>
      {results.content.listings.map((item, i) => (
       <div key={i} className='flex space-y-2 justify-between space-x-4 p-5 border rounded-lg'>
        <img src={item.url} alt={item.title} className='h-44 w-44 rounded-lg' />
